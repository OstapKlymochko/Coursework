...
namespace AuthService.Services
{
    public class JwtService : IJwtService
    {
        ...
        private string SignAuthToken(IdentityUser<int> user, IEnumerable<string?> roles, DateTime expirationTime)
        {
        ...
        {
            string accessToken = SignAuthToken(user, roles, DateTime.UtcNow.AddDays(1));
            string refreshToken = SignAuthToken(user, roles, DateTime.UtcNow.AddDays(2));
